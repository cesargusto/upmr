...
public class Moviments {
	...
	Solution best_sol_mov;
	private final int n_vizinhanca = 5;
	
	public Moviments() {
		this.best_sol_mov = null;
	}
	
	public boolean avalia_solucao(int best_makespan, Solution s_corrente, Solution s_melhor, ArrayList<Integer> alter_maqs, boolean teste) throws CloneNotSupportedException{
		
		int fo_1  = best_makespan;
		int fo_2 = Collections.max(s_corrente.makespan((ArrayList<Integer>) s_corrente.Tempos(), alter_maqs));
		
		//Right here the resource limits is tested
		
		boolean key = true;
		
		//key = s_corrente.check_feasibility();
	...
	public boolean avalia_solucao(int best_makespan, Solution s_corrente, Solution s_melhor, ArrayList<Integer> alter_maqs) throws CloneNotSupportedException{
		...
		int fo_1  = best_makespan;
		Solution s_copia = s_corrente.clone();
		int fo_2 = Collections.max(s_copia.makespan((ArrayList<Integer>) s_copia.Tempos(), alter_maqs));
		
		//Right here the resource limits is tested
		boolean intensive = true;
		if(intensive) {
			if(s_copia.check_feasibility()) {
				if(fo_2 < fo_1) {
					this.best_sol_mov = s_copia.clone();
					return true;
					}
				else
					return false;
			}else {
				//PROBLEM RIGHT HERE. IT DIDN'T REPAIR
				s_copia.repair_solution();
				if(s_copia.makespan() < fo_1) {
					this.best_sol_mov = s_copia.clone();
					return true;
				}
				else
	...
	public Solution troca_intra_Maq(Solution solucao) throws CloneNotSupportedException{
		...
		if(this.best_sol_mov == null) {
			this.best_sol_mov = solucao.clone();
		}
	...
	public Solution remove_job_Maq_mspan(Solution solucao) throws CloneNotSupportedException{
		...
		int best_mspan = this.best_sol_mov.makespan();
		ArrayList<Integer> alter_maqs = new ArrayList<>();
		...
		if(solucao.getMaq(maq_mspan).getSizeMaq() > 1) {
			for(int i = 0;i < solucao.getMaq(maq_mspan).getSizeMaq();i++){
